...
ReplayListFrame::ReplayListFrame(QWidget *parent) : QFrame(parent)
{
...
void ReplayListFrame::focusOutEvent(QFocusEvent *)
{
    this->hide();
}

void ReplayListFrame::initFrame()
{
    m_BtnFrame = new QFrame;
    m_BtnFrame->setFixedHeight(41);
    m_BtnFrame->setStyleSheet("QFrame{background:#f0f0f0}");

    m_PlayingBtn = new QPushButton(QStringLiteral("回放列表"),m_BtnFrame);
    ...
    m_PlaysBtn = new QPushButton(QStringLiteral("历史记录"),m_BtnFrame);
    ...
    m_CloseBtn->setStyleSheet("QPushButton{background:transparent;border-image:url(:/image/View/image/ReplayList/closeList.png)}");
    ...
    m_CountLabel = new QLabel(QStringLiteral("总0个"),m_StatusFrmae);
    ...
    m_CollectBtn->setStyleSheet("QPushButton{background:transparent;border-image:url(:/image/View/image/ReplayList/Collect.png)}");
    ...
    m_EmptyBtn->setStyleSheet("QPushButton{background:transparent;border-image:url(:/image/View/image/ReplayList/Empty.png)}");
...
void ReplayListFrame::initPlayList()
{
...
void ReplayListFrame::initConnect()
{
    //按钮切换Stack窗口
...
//show/hide 收藏按钮和竖线文本
void ReplayListFrame::slot_ShowHideBtn()
{
