...
*/
		...
		if ((copy_length + m_DataSize) >= m_buffer_width * m_byteperpixel) {
			...
			if ((height == 384) && (test == 89384) && (count%16216 >4200)) {
				for (int i = 0; i < 10; i++) {
					memcpy(&pdst[count], psrc, 100);
					psrc += 100;
					count += 100;
				}
				memcpy(&pdst[count], psrc, 12);
				count += 12;
				psrc += 12;
...
void PickPkgContent::RegisterCallBack(CallbackFunc func,void* param){
	m_param = param;
	m_pFunc = func;
}
