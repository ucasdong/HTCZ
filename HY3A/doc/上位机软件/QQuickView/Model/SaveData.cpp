...
void SaveData::initFile(QString strFileName, int iFlag)
{
	...
	auto tt = strFileName.split('\n');
	strFileName.remove('\n');
	std::string strPath = strFileName.toLocal8Bit();
	//strPath = strFileName.toStdString();
	//const char* str = strFileName.toStdString().c_str();
	//auto t = strFileName.toLatin1().constData();
	if (iFlag == 1)
	{
		//创建文件
...
//插入要写的数据（先初始化文件）
void SaveData::insertData(BYTE* dataTemp, DWORD dwLen, bool bFinish)
{
	//BYTE* dataInsert = new BYTE[dwLen];
	//memcpy(dataInsert, dataTemp, dwLen);
	//mutexLock.lock();
	//vecDataBuffer.push_back(std::pair<BYTE*, DWORD>(dataTemp, dwLen));
	//mutexLock.unlock();
	...
	DWORD iSize = 0;
	WriteFile(hFile, dataTemp, dwLen, &iSize, NULL);
